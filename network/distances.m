function [distancesMat] = distances(xy2, inNest, withNest)
    ...
    for i = 1:1:size(xy2, 1)
        for in = 1:1:size(xy2, 2)
            ...
            distancesMatTemp2 = reshape(distancesMatTemp1', size(inNestTemp, 2), size(inNestTemp, 2), []);      % organize the matrix into a n*n*num-of-frames matrix.
            for ind = 1:size(distancesMatTemp2, 1)
                distancesMatTemp2(ind, 1:ind, :) = fliplr(distancesMatTemp2(ind, 1:ind, :));        % delete the upper half of the matrix
            end
